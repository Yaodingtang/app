...
static int GetInputEventFromBuff(PInputEvent ptInputEvent)
{
...
void RegisterInputDevice(PInputDevice ptInputDevs)
{
	ptInputDevs->ptNext = g_InputDevs;
	g_InputDevs = ptInputDevs;
}
void InputInit(void)
{
...
int GetInputEvent(PInputEvent ptInputEvent)
{
	...
	/* 无数据则休眠 */
	pthread_mutex_lock(&g_tMutex);
	if (GetInputEventFromBuff(&tEvent))
	{
		*ptInputEvent = tEvent;
		pthread_mutex_unlock(&g_tMutex);
		return 0;
